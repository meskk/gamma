...
| P-4/A4f | 2026-07-09 | *(dieser Commit)* | fmt+clippy (on- und offline) +test --all ✓ (40 Suites, 1 neuer Write-Orakel-Test), sqlx-Cache regeneriert; fokussiertes Review des A4f-Deltas | Write-Orakel geschlossen: PostRepository::post_visible_to (Area-Prädikat only, kein hidden_at — ändert das report/interaction-Verhalten auf hidden Posts nicht) als Pre-Check. report() (posts/service) prüft post_visible_to(Some(reporter)) VOR dem Insert → Nicht-Berechtigter erhält 404 (ununterscheidbar vom Report auf nicht existierende Id; entitled darf reporten, ADR §5). record() (interactions/service, jetzt mit PostRepository-Feld) prüft bei POST-getargetetem Event (post_id gesetzt NACH Normalisierung) → 404; direkte User→User-Kante (target_id gesetzt) unberührt. Test report_and_interact_on_a_private_post_are_gated (privat-Fremd→404 = nicht-existent, entitled→204/201, direkt→201) |
