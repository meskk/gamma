...
| P-4/A1 | 2026-07-08 | 3632a08 | Docs; Owner-Scoping eingeholt (4 Antworten); Multi-Agent-Review (6 bestätigt, 17 widerlegt — Blocker Media-Pfad: Asset-Entitlement kennt keine Posts, Preis-0-Medien privater Posts wären für jeden abrufbar gewesen → Media-Rail explizit area-gated im ADR; Ökonomie-Frage entschieden: private Interaktionen speisen den Settlement-Graphen NICHT — Rail-Trennung statt Bot-Harvest hinter der Paywall; Ledger-Splice repariert) | P-4 entschieden + Stufenplan A1–A10 in §5; ADR 0011 (non-custodialer Payment-Seam): Kundengeld nie auf Plattform-Konten (Direct Charge + Application Fee = Firmenumsatz), Fiat berührt ledger_entries NIE (purchases-Audit-Spiegel, nicht konserviert), PaymentProvider-Trait als dritter Seam (Stripe Stufe 1, Wallet Stufe 2 hinter demselben Trait), Creator-gewähltes Zugangsmodell (free/one_time/subscription/per_post) + Entitlements mit expires_at statt Cron, Sichtbarkeits-Invariante fail-closed in Repository-Queries inkl. Media-Rail, Cut als econ-params-Knopf, Webhook als einzige Wahrheitsquelle, Flags + Rechts-Gate vor Aktivierung |
| P-4/A2 | 2026-07-09 | d7371b3 | fmt+clippy (on- und offline) +test --all ✓ (38 Suites, 4 neue Integrationstests), sqlx-Cache regeneriert; Multi-Agent-Review (4 Finder + adversariale Verifikation; 4 eingearbeitet, 7 widerlegt — Kern-Fix: grant_entitlement MONOTON, ein Grant verlängert nur noch [NULL=dauerhaft als +∞] — der dreifach gefundene Downgrade-Trap [permanenter Einmalkauf von späterem Abo-Event zerstört, Expiry-Rewind durch out-of-order-Stripe-Retries] ist damit im Schreibprimitiv selbst unmöglich, Widerruf wird eigener A6-Pfad; Composite-Anker (provider, provider_ref) im Test bewiesen [gleiche Ref unter anderem Provider = legitimer Kauf]; purchases-CHECKs in schema_guards_hold; is_entitled-Doc präzisiert [Entitlement-ROW, nicht volles A4-Prädikat — free-Areas haben keine Row]) | Datenmodell der Private Area: Migration 0021 (posts.area public/private, Default public — Bestand unverändert; private_areas mit Creator-gewähltem access_model + Integer-Cents + EUR-CHECK; area_entitlements mit expires_at-Semantik — Ablauf widerruft selbst, kein Cron, Grants nur verlängernd; purchases-Audit-Spiegel mit (provider, provider_ref)-UNIQUE als Webhook-Idempotenz-Anker + P-5-Indizes), private_area-Modul (Model/Repository: Upsert, is_entitled mit Creator-Selbst-Zugang, record_purchase mit Neu-Signal für Seiteneffekt-Skip), econ-params v3 (private_area_fee_bps=1000, validiert) |
| P-4/A3 | 2026-07-09 | 194a8dd | fmt+clippy (on- und offline) +test --all ✓ (39 Suites, 4 neue HTTP-Integrationstests), Bindings regeneriert + FE-Typecheck ✓; Multi-Agent-Review des Diffs (2 bestätigt + 3 Stimmen-verkürzte, inhaltlich echte Findings eingearbeitet: das Middleware-Gate lieferte bei Flag-aus ein JSON-404 + Allow-Header bei Methoden-Mismatch → per curl fingerprintbar; ersetzt durch KONDITIONALES Mounten [Flag-aus ⇒ Route nie gemountet ⇒ byte-identisch zu unrouted, im Test gegen eine echt nicht existierende Route bewiesen inkl. POST/DELETE/kein-Token]; Accept-Pfad-Matrix je Modell ergänzt) | Bereichs-Konfigurations-API hinter GAMMA_PRIVATE_AREA (default AUS, ADR 0011 §6): Flag einmalig beim Start gelesen, Routen werden nur bei AN gemountet — dark ist ununterscheidbar von nicht existent (kein Allow/JSON/401/400-Tell), ohne Env-Races testbar (AppState.with_private_area); PUT+GET /me/private-area (Creator-Id aus der Session, nie aus Body/Pfad) + GET /users/:id/private-area (die Terms SIND das öffentliche Angebot, nur eingeloggt); Service-Validierung mit stabilen Codes (unknown_access_model, negative_price, price_too_high — Cap €10k als Fat-Finger-Grenze, kein econ-Knopf —, missing_price für one_time/subscription, price_on_unpriced_model für free/per_post, description_too_long), DB-CHECKs bleiben Backstop; ts-rs-DTOs PrivateAreaConfigRequest/PrivateAreaView |
| A4-Design | 2026-07-09 | (kein Commit) | Multi-Agent-Design-Workflow (3 divergente Architekten + Vollständigkeits-Kritiker je Vorschlag + Synthese) statt direktem Bau — A4 ist der sicherheitskritischste P-4-Schritt (ein übersehener Read-Pfad = exponierter Privat-Content) | Bauspezifikation für A4a–A4g festgelegt: EIN kanonisches Viewer-Prädikat (public OR author OR live-entitlement OR free-mit-Login), fail-closed per NULL-Propagation; OptionalAuthUser der stale/kaputte Tokens zu anonym degradiert (nie neu-401, Infra-Fehler propagieren); Media-Rail via posts.media_id-Join mit public-rescue bei Mehrfach-Attach; Existenz-Orakel (404-vs-leer, Statuscodes) je Pfad geschlossen; 5 Owner-Defaults dokumentiert (free=Login-erforderlich, per_post deferred, Media-Doppel-Gate, Operator-Body-Review out-of-scope, Shared-Asset) |
| P-4/A4a | 2026-07-09 | bf87d47 | fmt+clippy (on- und offline) +test --all ✓ (39 Suites, 2 neue + 1 erweiterter Test), sqlx-Cache + Bindings regeneriert, FE-Typecheck ✓; Multi-Agent-Review (0 bestätigt; das eine Finding [create-Enqueue nicht area-gated] 0/3 widerlegt — Analyse-Gate ist A4bs anonymer get_post-Read, nicht der Enqueue; als Ordnungs-Note im Doc verankert) | Fundament der Sichtbarkeits-Invariante (verhaltensneutral bis A4g, da noch kein privater Post via API erzeugbar): posts.area als Post- und ReportedPost-Feld durch ALLE query_as!(Post)-Projektionen (get/list/create/set_hidden/Feed-CTEs, ein vergessenes SELECT = Compile-Fehler), Blanket area='public' in den 4 Ingestion/Status-Queries (private Posts verlassen die AI-Rail komplett), edges_for_epoch droppt private Posts INNERHALB des target_id-IS-NULL-Zweigs (direkte User→User-Kanten überleben, ein Top-Level-AND hätte alle Tip-Gems genullt), Invarianten-Doc um das Area-Prädikat + Media-Rail + Zukunfts-Note (M2.7-Ranker) erweitert; Tests: edges_exclude_private_posts (Like gedroppt, Comment-Direktkante überlebt), backfill_and_status_exclude_private_posts, area-Round-Trip |
| P-4/A4b | 2026-07-09 | cd93e34 | fmt+clippy (on- und offline) +test --all ✓ (40 Suites, 6 HTTP-Matrix-Tests), sqlx-Cache regeneriert; Multi-Agent-Review (4 Dimensionen inkl. Leak/Orakel-Jagd; 0 bestätigt; 2 widerlegte-aber-wertvolle Coverage-Guards freiwillig ergänzt: bezahlte Area NICHT sichtbar für eingeloggte Nicht-Berechtigte [pinnt den access_model='free'-Diskriminator gegen einen katastrophalen Leak], echt abgelaufener Session-Token → anonym-nicht-401) | Erstes echtes Read-Gate (Posts): OptionalAuthUser-Extractor (stale/kaputte/abgelaufene Tokens → anonym, NIE 401 — kein Regress für öffentliche Reads; nur Infra-Fehler propagieren, damit ein Berechtigter nicht still degradiert wird), durch get_post/list_posts Handler→Service→Repo gefädelt; das kanonische Viewer-Prädikat (public OR author OR live-entitlement OR free-mit-Login) inline in posts get/list, fail-closed per NULL-Propagation (anonym sieht nur public); privater Post = 404 ununterscheidbar von fehlender Id (kein Existenz-Orakel); Tests: privat unsichtbar für anon+Fremde/sichtbar für Creator, Entitlement gewährt + Ablauf widerruft, free=Login-erforderlich (Owner-Default), bezahlte-Area-Diskriminator, stale-Token + abgelaufene-Session = anon |
| P-4/A4c | 2026-07-09 | ee806c7 | fmt+clippy (on- und offline) +test --all ✓ (40 Suites, 1 neuer Feed-Test), sqlx-Cache regeneriert; fokussiertes Review des A4c-Deltas (Alias-Änderung, weggelassener Login-Guard, Service-Bypass — 0 Findings) | Feed-Kandidaten-Gate: dasselbe kanonische Prädikat (aus A4b) in alle drei Feed-CTEs (from_follows/from_category/from_popularity) mit $1=user_id als Viewer; da der Feed-Viewer strukturell immer eingeloggt ist (user_id: i64, kein Option), lässt der free-Arm den IS-NOT-NULL-Guard bewusst weg; die zwei bare FROM posts bekommen einen p-Alias; Gating der Kandidaten gated den ganzen Feed (der Ranker/Cursor liest nur die Kandidatenmenge); Test candidate_set_excludes_private_posts (bezahlt-privat vom gefolgten Autor ausgeschlossen, free-privat eingeschlossen, Entitlement lässt bezahlt zu) |
| P-4/A4d | 2026-07-09 | cfbcdf1 | fmt+clippy (on- und offline) +test --all ✓ (40 Suites, 1 neuer Kommentar-Test), sqlx-Cache regeneriert; fokussiertes Review der comment-spezifischen Orakel (leere-Liste-statt-404, create-Existenz-Orakel — 0 Findings) | Kommentar-Gate: list_for_post mit Viewer-Prädikat auf dem JOINten Post (anonym-fähig, mit Login-Guard) — privater Thread liefert leere 200-Liste, NICHT 404 (ununterscheidbar von öffentlichem Post ohne Kommentare); create-EXISTS-Guard um das Area-Prädikat mit $2=author_id erweitert (nur wer den Post SEHEN kann, darf kommentieren) → Nicht-Berechtigter erhält None→404, ununterscheidbar vom Kommentar auf eine nicht existierende Id (schließt das create-Erfolg-vs-404-Orakel); handler.list nimmt OptionalAuthUser; Test comments_on_a_private_post_are_gated (Thread unsichtbar für anon+Fremde/sichtbar für Creator+Berechtigten, Fremder-schreibt→404, Berechtigter→201) |
| P-4/A4e | 2026-07-09 | 37c57c5 | fmt+clippy (on- und offline) +test --all ✓ (40 Suites, 2 neue Media-Tests: Repo-Matrix + HTTP), sqlx-Cache regeneriert; volles Multi-Agent-Review (4 Dimensionen, 1 bestätigt/10 widerlegt) | Media-Rail — der Pfad, den hidden_at strukturell verfehlt (ADR 0011 §5): media_area_allows joint posts.media_id→owning post und erlaubt Zugriff nur bei unattached ODER ≥1 SICHTBAREM (hidden_at IS NULL) area-sichtbarem Post; „public rescue" bei Mehrfach-Attach; is_entitled faltet die Area-Prüfung ein (Raw-URL nie ohne Area-Zugang), get/unlock/manifest 404 VOR jedem status-verratenden Zweig (Metadaten-/free_content-/transcode-Orakel geschlossen). Review-Fix: das bestätigte Finding (runtergenommener Post streamte sein Media weiter) durch hidden_at auf dem Rescue-EXISTS geschlossen — korrigierte Form (nur Rescue-Arm, NICHT die unattached-NOT-EXISTS, sonst würde ein hidden Post fälschlich als unattached durchgelassen); Residual (Owner-Short-Circuit + Asset-Level-Takedown bei ein-Asset-viele-Posts) als eigener Moderations-Task ausgelagert. Tests: unattached/public/privat-Fremd-verweigert/Owner/Entitled/public-rescue/free/expired/taken-down + ready-transcodiert-Statusunabhängig |
| Media-Takedown | 2026-07-09 | a0d2a7e | fmt+clippy (on- und offline) +test --all ✓ (40 Suites, 2 neue Tests), sqlx-Cache regeneriert; fokussiertes Review (1 BLOCKER bestätigt + gefixt, 2 widerlegt) | Moderations-Follow-up zu A4e (eigener Schritt, NICHT in A4 gemischt): schließt beide Residuen. #1 Owner-Bypass — der Owner-Short-Circuit in den Media-Guards ENTFERNT: der Owner unterliegt jetzt media_area_allows (Area-Zugang via Author-Arm auf SICHTBAREM Post, aber Moderation via hidden_at), sodass ein runtergenommener Post sein Media auch dem Owner 404't (konsistent mit Text-Rail). #2 Asset-Level-Takedown — Migration 0022 (media_assets.hidden_at), gate()-Helfer 404't ein getaktetes Asset für ALLE inkl. Owner unabhängig von Post-Anzahl, Operator-Endpoints POST /media/:id/takedown+restore (AdminUser). Review-BLOCKER gefixt: finalize()+transcode() umgingen gate() und minteten via view(_,true) eine Raw-URL zu getaktetem Media — hidden_at-Guard in beide eingezogen (Asset ist eingefroren). Tests: Asset-Takedown blockt alle inkl. Owner auf get/unlock/manifest/finalize/transcode + non-op→403 + restore; Owner-eigener-getakteter-Post-Media→404 |
| P-4/A4f | 2026-07-09 | 6f1fd4c | fmt+clippy (on- und offline) +test --all ✓ (40 Suites, 1 neuer Write-Orakel-Test), sqlx-Cache regeneriert; fokussiertes Review des A4f-Deltas (0 Findings) | Write-Orakel geschlossen: PostRepository::post_visible_to (Area-Prädikat only, kein hidden_at — ändert das report/interaction-Verhalten auf hidden Posts nicht) als Pre-Check. report() (posts/service) prüft post_visible_to(Some(reporter)) VOR dem Insert → Nicht-Berechtigter erhält 404 (ununterscheidbar vom Report auf nicht existierende Id; entitled darf reporten, ADR §5). record() (interactions/service, jetzt mit PostRepository-Feld) prüft bei POST-getargetetem Event (post_id gesetzt NACH Normalisierung) → 404; direkte User→User-Kante (target_id gesetzt) unberührt. Test report_and_interact_on_a_private_post_are_gated (privat-Fremd→404 = nicht-existent, entitled→204/201, direkt→201) |
| P-4/A4g | 2026-07-09 | b928e44 | fmt+clippy (on- und offline) +test --all ✓ (40 Suites, 3 neue Tests), sqlx-Cache + Bindings regeneriert, FE-Typecheck ✓; fokussiertes Review des A4g-Deltas | Der Schreibpfad (FINAL, macht privaten Post erstmals via API erzeugbar — bewusst zuletzt, nach allen Read-Gates): NewPost.area (serde-Default public, ts-rs-Binding regeneriert), create-INSERT trägt area, Service whitelistet area∈{public,private}→invalid_area-400 (DB-CHECK bleibt Backstop), Ingestion-Enqueue wird bei area≠public übersprungen (private Posts nie analysiert). FLAG-Entscheidung (ADR §6): GAMMA_PRIVATE_AREA gated Config/Checkout/Webhook, NICHT die Post-Erstellung — A4g ist flag-UNABHÄNGIG (Read-Invariant schützt ohnehin flag-unabhängig; Frontend-Flag versteckt nur den Reiter). 16 NewPost-Test-Literale um area ergänzt. Tests: private-Post-nie-enqueued (public schon), invalid_area→400, POST /posts area=private bei Flag-AUS→201+Creator-sichtbar/Fremd-404 |
| P-4/A4 Fable-Review | 2026-07-10 | *(dieser Commit)* | fmt+clippy (on/offline) +test --all ✓; Fable-Gesamtreview des Range 3632a08..b928e44 (4 Dimensionen holistisch, schrittübergreifend; 4 bestätigt / 4 widerlegt) | Frischer Blick auf den GESAMTEN gestrigen A2–A4g-Block fand eine echte schrittübergreifende Lücke, die kein Einzel-Review sehen konnte: finalize()/transcode_owned() umgingen gate() und minteten via view(_,true) weiterhin eine Raw-URL zu Media eines runtergenommenen POSTS (der Media-Takedown-Fix a0d2a7e deckte nur Asset-Level-Takedown) → beide HTTP-Owner-Pfade jetzt durch gate() geleitet (Asset UND Post-Takedown), Worker-transcode() bleibt auf asset.hidden_at (verwirft die View), Regressionstest ergänzt. Doku/Ledger-Genauigkeit: ADR §5 um den free-Arm + Login-Guard-Nachtrag ergänzt (Code zitierte den ADR, der es nie sagte), Invarianten-Doc-Formulierung zum Interaction-Write-Pfad korrigiert (Area- nicht hidden_at-Guard), drei alte Ledger-Platzhalter nachgetragen (A4g b928e44, M2.3 07dcf4e, M4.6 3e58661) |
| Like-Lifecycle (ADR 0012) | 2026-07-17 | c7d1d68 + 3b009ee | fmt+clippy (on/offline) +test --all ✓ (40 Suites, 10 neue Tests), sqlx-Cache + Bindings regeneriert; FE typecheck+lint+test ✓ (60 Tests); adversarialer 4-Linsen-Multi-Agent-Review (9 bestätigte Funde, alle gefixt: COUNT(DISTINCT actor) statt Journal-Zeilen, Ein-Query-Kommentar-Guard gegen Timing-Orakel, Reels-Scrollback-State-Lifting, Revert-Key-Guard, 2 Test-Pins); UI end-to-end im Browser verifiziert (Like/Unlike/Reload-Hydration/Scrollback/Profil) | Alle fünf Like-Lücken geschlossen: (1) liked_by_me-Hydration (die dokumentierte „UI-Lüge“ ist weg), (2) like_count auf Post/Feed/Comment + likes_received am Profil — live als DISTINCT Liker aus dem Journal, (3) Unlike als Retraktion (retracted_at-Voiding, append-only bleibt; DELETE /v1/interactions idempotent; Re-Like ent-void via ON CONFLICT), (4) Kommentar-Likes (comment_id-Target, Kante zum Kommentar-Autor, Sichtbarkeits-Gates in Write+Settlement) + Glass-Profile-Like-Badges/Stat, (5) Feed-Ranker: ln(1+likes)-Term + Popularitäts-CTE nach aktiven Likes statt toter popularity_score-Spalte. Migration 0024; ADR 0012 |
| Follower-Count | 2026-07-17 | 0288f9e | fmt+clippy (on/offline) +test --all ✓ (1 neuer Backend-Test), sqlx-Cache + Bindings regeneriert; FE typecheck+test ✓ (61 Tests, 1 neuer); fokussierter 2-Linsen-Review (0 Funde); UI verifiziert (Stat + optimistisches ±1 beim Folgen/Entfolgen) | Profil-Datenlücke geschlossen: User.followers_count (live COUNT über follows.followee_id — bislang gab es nur die Following-Liste), Glass-Profile-Statszeile zeigt „Follower“ mit sofortigem ±1 beim eigenen Follow-Toggle (Baseline+Override-Muster wie useLike) |
...
- **Free-Area-Sichtbarkeit (ENTSCHIEDEN 2026-07-09, Owner):** Ein `access_model='free'`
  Bereich ist NUR für eingeloggte Nutzer sichtbar (Mitglieder ohne Bezahlung,
  Login erforderlich), NICHT weltöffentlich. Im A4-Prädikat als `$V IS NOT NULL`
  im free-Arm umgesetzt (Posts/Feed/Kommentare/Media konsistent). Konsequenz:
  free-Area-Posts erscheinen für jeden eingeloggten Nutzer auch in der globalen
  Timeline. Fail-closed; später zu weltöffentlich öffenbar durch Entfernen des
  Guards, falls der Owner es umdreht.
